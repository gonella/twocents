package com.twocents.test.report.dynamicjasper;
...
public class BarChartBuilderTest extends BaseDjReportTest {
	...
	protected void setUp() throws Exception {
		...
		valueAxisFormat.setRangeMaxValueExpression(new NumberExpression(
				new Integer(100000)));
		...
		DJChart djChart = new DJBarChartBuilder()
				...
				.setTitleFont(Font.ARIAL_BIG_BOLD).setSubtitle("subtitle")
				...
				// plot
				.setShowTickMarks(true).setShowTickLabels(true)
	...
	public void testChart() {
		...
		assertEquals(new Byte(DJChartOptions.EDGE_BOTTOM),
				chart.getLegendPositionByte());
		assertEquals(new Byte(DJChartOptions.EDGE_TOP),
				chart.getTitlePositionByte());
		assertEquals(new Byte(DJChartOptions.LINE_STYLE_DOTTED), chart
				.getLineBox().getPen().getLineStyle());
		assertEquals(new Float(1), chart.getLineBox().getPen().getLineWidth());
		assertEquals(Color.DARK_GRAY, chart.getLineBox().getPen()
				.getLineColor());
		assertEquals(new Integer(5), chart.getLineBox().getPadding());
	}

	public void testDataset() {
		JRDesignCategoryDataset dataset = (JRDesignCategoryDataset) chart
				.getDataset();
