...
public class CrosstabReportBrokerageItem extends BaseReport {
	...
	public CrosstabReportBrokerageItem(List dataCollection){
	 ...
	 */
		FastReportBuilder drb = new FastReportBuilder();
			drb
			.setTitle("TWOCENTS")
			.setSubtitle("Relatório gerado às [" + new Date()+"]")
		...
		if(!CollectionUtils.isEmpty(dataCollection)){
			params.put("sr", SortUtils.sortCollection(dataCollection,djcross));
		}
	...
	private DJCrosstab createCrosstab() {
		...
		cb.addMeasure("amount",Integer.class.getName(), DJCalculation.SUM , "Amount",measureStyle);

		DJCrosstabRow row = new CrosstabRowBuilder().setProperty("dateMonthAndYear",String.class.getName())
			.setHeaderWidth(100).setHeight(20)
			.setTitle("dateMonthAndYear")
		...
		row = new CrosstabRowBuilder().setProperty("dateDayAndMonthAndYear",String.class.getName())
			.setHeaderWidth(100).setHeight(20)
			.setTitle("dateDayAndMonthAndYear").setShowTotals(true)
		...
		row = new CrosstabRowBuilder().setProperty("stockCode",String.class.getName())
			.setHeaderWidth(100).setHeight(30)
			.setTitle("stockCode").setShowTotals(true)
			.setTotalStyle(totalStyle).setTotalHeaderStyle(totalHeader)
			.setHeaderStyle(colAndRowHeaderStyle)
			.build();
		...
		cb.addRow("title","averageBuyPrice",Double.class.getName(),true);
		
		
		DJCrosstabColumn col=null; 
		
/*		col = new CrosstabColumnBuilder().setProperty("state",String.class.getName())
		...
		col = new CrosstabColumnBuilder().setProperty("daytrade",Boolean.class.getName())
			.setHeaderHeight(30).setWidth(70)
			.setShowTotals(true).setTitle("daytrade")
			.setTotalStyle(totalStyle).setTotalHeaderStyle(totalHeader)
			.setHeaderStyle(colAndRowHeaderStyle)
			.build();
...
*/
	...
	public static void main(String[] args) throws Exception {
		List dataCollection = MockProduct.getDummyCollection();

		CrosstabReportBrokerageItem test = new CrosstabReportBrokerageItem(dataCollection);
		test.testReport();
